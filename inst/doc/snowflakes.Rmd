...
---
If you are tired of the point types available in R base graphics and want to break your report routine, you can use package `snowflakes`. The package contains one function that plots random snowflakes that can be used instead of points. The snowflakes have some useful properties. Just like points in R base graphics, the snowflakes are round, no matter what the aspect ratio you are using. Another nice property is that the snowflakes are scaleable: if you increase the size of the snowflake, its cristal width will increase accordingly. Most importantly, there is an infinite number of snowflake patterns that you can choose from. Each pattern is uniquely determined by a random seed. Snowflakes can be plotted using different sizes, widths, angles, and of course, colors. The following examples demonstrate how to create all kinds of cool snow-looking snowflakes.
...
First, let's create a simple plot. Suppose we need to plot a quadratic function, $Y = (X-1)^2$. Instead of points, we use snowflakes plotted with transparent gray color. Transparent colors make snowflakes look more like crystals.
...
Note that in the above plot, every snowflake is unique. The pattern of each snowflake is defined by a random seed. The seeds used in generating the above plot were returned by the function in case you want to reproduce some of the patterns. 

## Exploring other arguments

The next example demonstrates the use of argument `deltaCoef`. It defines the `crystal` width of each snowflake.
...
Here is another example. This time, we only use three snowflake patterns with seeds `1`, `2`, and `3`. We also vary the orientation and the color of the snowflakes. For colors, we use opaque grays: the color of the frame of the snowflake does not change, while the color of the details is getting lighter. Also in this example, we modified the aspect ratio argument of function `plot`, but the snowflakes are still round. Note that each snowflake is facing the center of the spiral because we chose argument `orientation` this way.  You can see however, that the orientation is not exact. This happens because I need to take the aspect ratio into account when computing the orientation (future work).
...
Note that in the example above, we used opaque gray colors. If we used a transparent color instead (as in previous examples), the snowflakes would look more interesting, which is demonstrated by the next example.
...
And now let's plot some cool snowflakes and their seeds:
